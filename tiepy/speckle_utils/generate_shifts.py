...
from speckle_utils.template_matching import match_template
from speckle_utils.template_utils import get_windows, generate_gaussian_mask
from numpy import fft
from speckle_utils.phase_retrieval import kottler
 



def process(I0, In, method = match_template):
    
    windows, c = get_windows(I0, 50, 4)
    print(len(windows))
    ...
    for sel in tqdm(range(len(windows))): ### itr over window center
        ...
        for i in (range(I)): ### itr over pulse (80 kHz)
            for k in (range(K)): ### itr over train (0.1Hz)
                ...
                mask = generate_gaussian_mask(*image_crop.shape, c[sel][0], c[sel][1], templ.shape[0], 10).astype('float32')
